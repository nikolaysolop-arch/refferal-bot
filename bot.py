...
ADMIN_ID = 6127276408
PROVIDER_TOKEN = "ВАШ_ТОКЕН_ПРОДАЙЦА"  # Вставьте токен от BotFather
...
def pre_checkout(update: Update, context):
    ...
    if query.invoice_payload == "pro_subscription":
...
def successful_payment(update: Update, context):
    ...
    update.message.reply_text(
        ...
        "👑 Теперь ты получаешь:\n"
        f"• Увеличенные награды за задания (в 2 раза)\n"
...
def handle_buttons(update: Update, context):
    ...
    if text == f"💰 Баланс{pro_badge}" or text == "💰 Баланс":
        ...
        update.message.reply_text(
            ...
            f"👑 PRO до: {pro_until if pro_until != '—' else 'нет'}\n\n"
        ...
        if is_pro:
            update.message.reply_text(
                "👑 <b>У тебя уже есть PRO-подписка!</b>\n\n"
                f"Действует до: {row[7]}\n\n"
                "Спасибо за поддержку!",
                parse_mode="HTML",
                reply_markup=get_main_keyboard(uid)
            )
...
def handle_withdraw_details(update: Update, context):
    ...
    # Отправляем админу заявку с реквизитами
    admin_text = f"""
...
"""
    update.message.reply_text(confirm_text, parse_mode="HTML", reply_markup=get_main_keyboard(uid))
...
👇 <b>Нажми на кнопки внизу и зарабатывай!</b>"""
    ...
    # ==================== ВЫВОД СРЕДСТВ ====================
    elif data == "withdraw_sbp":
        context.user_data['withdraw_method'] = 'sbp'
        context.user_data['awaiting_withdraw_details'] = True
        query.edit_message_text(
            f"📱 <b>ВЫВОД НА КАРТУ ЧЕРЕЗ СБП</b>\n\n"
            f"💰 Сумма: {context.user_data.get('withdraw_amount', 0)} ₽\n\n"
            f"📝 <b>Введите номер телефона, привязанный к банковской карте:</b>\n\n"
            f"Пример: <code>+79123456789</code>",
            parse_mode="HTML"
        )
        ...
        query.edit_message_text(
            ...
            f"💰 Сумма: {context.user_data.get('withdraw_amount', 0)} ₽\n\n"
            f"📝 <b>Введите номер карты (16 цифр) и название банка:</b>\n\n"
            f"Пример: <code>1234 5678 9012 3456, Тинькофф</code>",
            parse_mode="HTML"
        )
...
def pro_off_command(update: Update, context):
    ...
    try:
        ...
        try:
            context.bot.send_message(user_id, "⚠️ Администратор отключил PRO-подписку. Награды вернулись к обычному уровню.")
        except:
            pass
    except:
...
if __name__ == "__main__":
    ...
    print("Бот с PRO-подпиской и выводом на СБП/карту/крипту/Stars запущен!")
    updater.idle()
